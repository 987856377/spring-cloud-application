...
import com.cloud.payment.module.payment.service.impl.PaymentHystrixServiceImpl;
...
public class PaymentController {
    ...
    public ResultJson getPaymentInfo(@PathVariable Long id){
        Payment paymentInfo = paymentHystrixService.getPaymentInfo(id);
        if (paymentInfo == null){
            return ResultJson.failure(ResultCode.GONE);
        }
        return ResultJson.success(paymentInfo);
    }

    @RequestMapping("/getPaymentInfoTimeOut/{id}")
    public ResultJson getPaymentInfoTimeOut(@PathVariable Long id){
        Payment paymentInfo = paymentHystrixService.getPaymentInfoTimeOut(id);
        if (paymentInfo == null){
            return ResultJson.failure(ResultCode.GONE);
        }
        return ResultJson.success(paymentInfo);
    ...
    public ResultJson paymentCircuitBreaker(@PathVariable Long id){
        Payment paymentInfo = paymentHystrixService.paymentCircuitBreaker(id);
        if (paymentInfo == null){
            return ResultJson.failure(ResultCode.GONE);
        }
        return ResultJson.success(paymentInfo);
