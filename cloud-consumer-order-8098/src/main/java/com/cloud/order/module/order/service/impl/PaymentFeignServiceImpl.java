...
public class PaymentFeignServiceImpl implements PaymentFeignService {    // 服务调用失败,指定 fallback 方法
    ...
    @Override
    public ResultJson create(Payment payment) {
        logger.error("[cloud-order-service] Invoke --- create --- SERVICE UNAVAILABLE");
        return ResultJson.failure(ResultCode.TIME_OUT);
    }
